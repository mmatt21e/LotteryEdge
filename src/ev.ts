 ...
 *
 * For any prize tier, published odds "1 in X" imply the whole run:
 *   tickets ≈ odds * originalCount.
 * Every tier should yield the same number, so we take the median across all
 * tiers that publish odds — robust to a single mis-published/rounded tier.
 *
 * If no tier publishes odds we cannot estimate the run from this data.
 */
export function estimateOriginalTickets(tiers: PrizeTier[]): number {
